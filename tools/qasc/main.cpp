...
static QByteArray combinePath(const QString &infile, const QString &outfile) {
    ...
    const QByteArray relativePath =
        QFile::encodeName(outFileInfo.dir().relativeFilePath(inFileInfo.filePath()));
#ifdef Q_OS_WIN
...
int runMoc(int argc, char **argv) {
    ...
    includePathOption.setDescription(
        QStringLiteral("Add dir to the include path for header files."));
    ...
    macFrameworkOption.setDescription(
        QStringLiteral("Add Mac framework to the include path for header files."));
    ...
    preprocessOption.setDescription(
        QStringLiteral("Preprocess only; do not generate meta object code."));
    ...
    compilerFlavorOption.setDescription(
        QStringLiteral("Set the compiler flavor: either \"msvc\" or \"unix\"."));
    ...
    prependIncludeOption.setDescription(
        QStringLiteral("Prepend #include <file> (preserve default include)."));
    ...
    includeOption.setDescription(
        QStringLiteral("Parse <file> as an #include before the main source(s)."));
    ...
    noNotesWarningsCompatOption.setDescription(
        QStringLiteral("Do not display notes (-nn) or warnings (-nw). Compatibility option."));
    ...
    noWarningsOption.setDescription(
        QStringLiteral("Do not display warnings (implies --no-notes)."));
    parser.addOption(noWarningsOption);

    QCommandLineOption ignoreConflictsOption(QStringLiteral("ignore-option-clashes"));
    ignoreConflictsOption.setDescription(
        QStringLiteral("Ignore all options that conflict with compilers, like -pthread conflicting "
                       "with moc's -p option."));
    parser.addOption(ignoreConflictsOption);

    QCommandLineOption jsonOption(QStringLiteral("output-json"));
    jsonOption.setDescription(
        QStringLiteral("In addition to generating C++ code, create a machine-readable JSON file in "
                       "a file that matches the output file and an extra .json extension."));
    parser.addOption(jsonOption);

    QCommandLineOption collectOption(QStringLiteral("collect-json"));
    collectOption.setDescription(
        QStringLiteral("Instead of processing C++ code, collect previously generated JSON output "
                       "into a single file."));
    ...
    if (compilerFlavor.isEmpty() || compilerFlavor == QLatin1String("unix")) {
        ...
        const auto cplus_include_path =
            qgetenv("CPLUS_INCLUDE_PATH").split(QDir::listSeparator().toLatin1());
    ...
    if (filename.isEmpty()) {
        ...
        if (!in.open(QIODevice::ReadOnly)) {
            fprintf(stderr, "qasc:%s: No such file\n", qPrintable(filename));
            return 1;
        }
    ...
    if (output.size()) { // output file specified
...
#endif
        {
            fprintf(stderr, "qasc:Cannot create %s\n", QFile::encodeName(output).constData());
            return 1;
        }
        ...
        if (parser.isSet(jsonOption)) {
            ...
            FILE *f;
#if defined(_MSC_VER)
            if (_wfopen_s(&f, reinterpret_cast<const wchar_t *>(jsonOutputFileName.utf16()),
                          L"w") != 0)
#else
            f = fopen(QFile::encodeName(jsonOutputFileName).constData(), "w");
            if (!f)
#endif
                fprintf(stderr, "qasc:Cannot create JSON output file %s. %s\n",
                        QFile::encodeName(jsonOutputFileName).constData(), strerror(errno));
            jsonOutput.reset(f);
        }
    } else { // use stdout
        out = stdout;
    ...
    if (parser.isSet(depFileOption)) {
        ...
        if (parser.isSet(depFilePathOption)) {
            ...
            fprintf(stderr, "qasc:Writing to stdout, but no depfile path specified.\n");
        }
        ...
        if (!depFileHandleRaw)
#endif
            fprintf(stderr, "qasc:Cannot create dep output file '%s'. %s\n",
                    QFile::encodeName(depOutputFileName).constData(), strerror(errno));
        depFileHandle.reset(depFileHandleRaw);

        if (!depFileHandle.isNull()) {
            // First line is the path to the generated file.
            fprintf(depFileHandle.data(),
                    "%s: ", escapeAndEncodeDependencyPath(depRuleName).constData());
